...
def inception_v3_base(inputs,
  ...
  bn_gn = ((name, #param
    {'beta' : tf.constant_initializer(param[0]), 'gamma': tf.constant_initializer(param[1]),
  ...
  def test_pdb(i=0):
    ...
    sess.run(init_op)
    print((param_dict[pm_names_order[i]][0].transpose(3,2,1,0) == sess.run(tf.trainable_variables()[2*i])).all())
    pdb.set_trace()
    sess.close()
    ...
    with arg_scope(
      ...
      with arg_scope(
        ...
        with variable_scope.variable_scope('Logits'):
          ...
          # 1000
        end_points['Logits'] = logits
        tsn_feat = logits#orig_net#end_points['Mixed_6e']#logits
        end_points['Predictions'] = prediction_fn(logits, scope='Predictions')
